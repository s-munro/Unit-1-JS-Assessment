...
function getStarshipPassengerAndCrewSumTotal(character) {
  ...
  let totalPass = character.starships.reduce(function (acc, item) {
    return acc + item.passengers;
  }, 0);
  return totalCrew + totalPass;
}
...
function getNthFilm(character, filmNumber) {
  if (filmNumber < 1 || filmNumber > 3) {
    return 'error, there are only 3 Star Wars Movies. I love the movie Ratatouille, though.';
  } else {
